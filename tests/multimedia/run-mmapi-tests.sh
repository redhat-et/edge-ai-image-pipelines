...
# speed things up.
#
# Source: https://github.com/OE4T/meta-tegra-community/blob/97813c33460513aad02ae65509954ad32f3d26bf/recipes-test/tegra-tests/tegra-mmapi-tests/run-mmapi-tests.sh

SAMPLEROOT="/usr/src/jetson_multimedia_api"
...
for d in /usr/src/jetson_multimedia_api/samples/*; do
    PATH="$PATH:$d"
done

for d in /usr/src/jetson_multimedia_api/samples/unittest_samples/*; do
    PATH="$PATH:$d"
done
...
run_video_decode() {
    ...
    video_decode H264 --disable-rendering "$vid"
}
...
run_video_dec_cuda() {
    ...
    if [ -n "$2" ]; then
	video_dec_cuda "$vid" H264 --disable-rendering --bbox-file "$2"
    else
	video_dec_cuda "$vid" H264 --disable-rendering
    fi
}
