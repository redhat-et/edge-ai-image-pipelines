    ...
    def test_driver_qemu(tmp_path, client):
        with client.log_stream():
            ...
            with FabricAdapter(
                ...
                ssh.sudo("ls -R") 
                ssh.sudo(
                        "podman run --name server --network triton -p8000:8000 --rm -d --device nvidia.com/gpu=all --ipc=host -v /:/share nvcr.io/nvidia/tritonserver:25.07-py3-igpu tritonserver --model-repository=/share/models"
                )
                ssh.sudo(
                        "podman run --name client --network triton --rm -it -v .:/share nvcr.io/nvidia/tritonserver:25.07-py3-igpu /bin/bash /share/triton-client.sh"
                )
            client.power.off()
