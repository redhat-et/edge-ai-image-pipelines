    ...
    def test_vllm(tmp_path, client):
        with client.log_stream():
            ...
            with FabricAdapter(
                ...
                ssh.sudo(
                        f"podman run --name client --network vllm --rm -it -v .:/share quay.io/redhat-user-workloads/octo-edge-tenant/jetson-wheels-vllm-app:latest /bin/bash /share/vllm-client.sh"
                )
            client.power.off()
